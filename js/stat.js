...
window.renderStatistics = function (ctx, names, times) {
  ...
  drawRect(dataCloud.startX + dataCloud.shadow, dataCloud.startY + dataCloud.shadow, dataCloud.widthRect, dataCloud.heightRect, dataCloud.colorRect[0]);
  ...
  function writeText(textArray) {
    ...
    for (var i = 0; i < textArray.length; i++) {
      ctx.fillText(textArray[i], dataCloud.startX + dataCloud.gap, dataCloud.startY + (i + 1) * 25);
    }
  }
  ...
  function drawHistogram(arrayTimes, arrayNames) {
    ...
    var initialX = dataCloud.startX + dataCloud.gap;
